...
window.jQuery(document).ready(function () {
  if (Laravel.groupId) {
    ...
    Echo.private('users.group.notification.' + Laravel.groupId).listen('PandaGroupCommentNotificationCreated', function (data) {
      window.jQuery('*[data-comment="' + data.comment.commentable_id + '"]').prepend(commentToAppend(data));
    }).listen('PandaNewActivityCreated', function (data) {
...
(function (window) {
  ...
  Player = function Player(playerNode) {
    ...
    function getTime(msec, useString) {
      ...
      var nSec = Math.floor(msec / 1000),
          ...
          sec = Math.floor(nSec - hh * 3600 - min * 60); // if (min === 0 && sec === 0) return null; // return 0:00 as null
      ...
      var sound = soundManager.createSound({
        ...
        whileplaying: function whileplaying() {
          ...
          if (this.duration) {
            ...
            dom.progressBar.style.width = width; // TODO: only write changes
        ...
        onbufferchange: function onbufferchange(isBuffering) {
          if (isBuffering) {
        ...
        onerror: function onerror() {
          ...
          callback('error', this); // load next, possibly with delay.
        ...
        onfinish: function onfinish() {
          ...
          callback('finish', this); // next track?

          item = playlistController.getNext(); // don't play the same item over and over again, if at end of playlist (excluding single item case.)
    ...
    function playLink(link) {
      ...
      if (soundManager.canPlayURL(link.href)) {
        ...
        setTitle(link.parentNode); // reset the UI
    ...
    function PlaylistController() {
      ...
      function getItem(offset) {
        var list, item; // given the current selection (or an offset), return the current item.
        ...
        list = getPlaylist(); // use offset if provided, otherwise take default selected.

        offset = offset !== undefined ? offset : data.selectedIndex; // safety check - limit to between 0 and list length
        ...
        if (data.playlist.length > 1) {
          if (data.selectedIndex >= data.playlist.length) {
            if (data.loopMode) {
              ...
              // no change
              data.selectedIndex--; // end playback
      ...
      function select(item) {
        var offset, itemTop, itemBottom, containerHeight, scrollTop, itemPadding, liElement; // remove last selected, if any
      ...
      function initPlaylistController() {
        ...
        initDOM();
        refreshDOM(); // animate playlist open, if HTML classname indicates so.
    ...
    function getActionData(target) {
      ...
      actionData.volume.height = target.offsetHeight; // potentially dangerous: this should, but may not be a percentage-based value.

      actionData.volume.backgroundSize = parseInt(utils.style.get(target, 'background-size'), 10); // IE gives pixels even if background-size specified as % in CSS. Boourns.
    ...
    function handleMouseDown(e) {
      ...
      if (utils.css.has(target, 'sm2-volume-control')) {
        ...
        utils.events.add(document, 'touchend', actions.releaseVolume); // and apply right away
    ...
    function handleMouse(e) {
      ...
      if (sound && sound.duration) {
        sound.setPosition(sound.duration * newPosition); // a little hackish: ensure UI updates immediately with current position, even if audio is buffering and hasn't moved there yet.
    ...
    function init() {
      ...
      dom.o = playerNode; // are we dealing with a crap browser? apply legacy CSS if so.
      ...
      dom.volume = utils.dom.get(dom.o, 'a.sm2-volume-control'); // measure volume control dimensions
    ...
    actions = {
      play: function play(offsetOrEvent) {
        /**
        ...
        soundObject.togglePause(); // special case: clear "play next" timeout, if one exists.
      ...
      /* e */
      {
        var item, lastIndex; // special case: clear "play next" timeout, if one exists.
        ...
        lastIndex = playlistController.data.selectedIndex;
        item = playlistController.getNext(true); // don't play the same item again
      ...
      /* e */
      {
        var item, lastIndex;
        lastIndex = playlistController.data.selectedIndex;
        item = playlistController.getPrevious(); // don't play the same item again
      ...
      menu: function menu(ignoreToggle) {
        var isOpen;
        isOpen = utils.css.has(dom.o, css.playlistOpen); // hackish: reset scrollTop in default first open case. odd, but some browsers have a non-zero scroll offset the first time the playlist opens.
      ...
      adjustVolume: function adjustVolume(e) {
        ...
        backgroundMargin = (100 - actionData.volume.backgroundSize) / 2; // relative position of mouse over element

        value = Math.max(0, Math.min(1, (clientX - actionData.volume.x) / actionData.volume.width));
        target.style.clip = 'rect(0px, ' + actionData.volume.width * value + 'px, ' + actionData.volume.height + 'px, ' + actionData.volume.width * (backgroundMargin / 100) + 'px)'; // determine logical volume, including background margin

        pixelMargin = backgroundMargin / 100 * actionData.volume.width;
        volume = Math.max(0, Math.min(1, (clientX - actionData.volume.x - pixelMargin) / (actionData.volume.width - pixelMargin * 2))) * 100; // set volume
      ...
      releaseVolume: function releaseVolume()
      /* e */
      {
      ...
      setVolume: function setVolume(volume) {
        ...
        if (dom.volume) {
          target = dom.volume; // based on getStyle() result

          backgroundSize = actionData.volume.backgroundSize; // figure out spacing around background image based on background size, eg. 60% background size.
          // 60% wide means 20% margin on each side.

          backgroundMargin = (100 - backgroundSize) / 2; // margin as pixel value relative to width
    ...
    init(); // TODO: mixin actions -> exports
  ...
  utils = {
    ...
    css: function () {
      ...
      function toggleClass(o, cStr) {
        ...
        method(o, cStr); // indicate the new state...
    ...
    dom: function () {
      function getAll(param1, param2) {
        ...
        if (arguments.length === 1) {
          // .selector case
          node = document.documentElement; // first param is actually the selector
      ...
      /* parentNode, selector */
      {
        var results = getAll.apply(this, arguments); // hackish: if an array, return the last item.
      ...
      function ancestor(nodeName, element, checkCurrent) {
        ...
        nodeName = nodeName.toUpperCase(); // return if current node matches.
    ...
    events: function () {
      ...
      add = function add(o, evtName, evtHandler) {
    ...
    features: function () {
      ...
      localAnimationFrame = window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame || window.oRequestAnimationFrame || window.msRequestAnimationFrame || null; // apply to window, avoid "illegal invocation" errors in Chrome
...
__webpack_require__(/*! ./LikeButton/LikeButton.js */ "./js/LikeButton/LikeButton.js");

__webpack_require__(/*! ./copyLinkButton/copyLinkButton.js */ "./js/copyLinkButton/copyLinkButton.js");

__webpack_require__(/*! ./Broadcasting/comments.js */ "./js/Broadcasting/comments.js");

__webpack_require__(/*! ./bar-ui.js */ "./js/bar-ui.js");
...
module.exports = __webpack_require__(/*! /Users/jasonhoendervanger/PhpstormProjects/deraad/packages/exdeliver/causeway/assets/js/website.js */"./js/website.js");
