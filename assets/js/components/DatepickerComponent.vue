<template>
    <div :style='{width:width}' class="datetime-picker" v-on:click='calendarClicked($event)'>
        <div>
            <input :name='name' :value="date" autocomplete='off' id='tj-datetime-input' type='text' v-on:click='toggleCal'/>
            <div :class='{noDisplay: hideCal}' class='calender-div'>
                <div :class='{noDisplay: hideDate}'>
                    <div class='year-month-wrapper'>
                        <div class='month-setter'>
                            <button class='nav-l' type='button' v-on:click='leftYear'>&#x3C;</button>
                            <span class='year'>{{year}}</span>
                            <button class='nav-r' type='button' v-on:click='rightYear'>&#x3E;</button>
                        </div>
                        <div class='month-setter'>
                            <button class='nav-l' type='button' v-on:click='leftMonth'>&#x3C;</button>
                            <span class='month'>{{month}}</span>
                            <button class='nav-r' type='button' v-on:click='rightMonth' v-on:mousedown=''>&#x3E;</button>
                        </div>
                    </div>
                    <div class='headers'>
              ...
              <span :class='{activePort: (weekIndex*7 + dayIndex) === activePort}' class="port" v-for="(day, dayIndex) in week" v-on:click='setDay(weekIndex*7 + dayIndex, day)'>
                {{day}}
              </span>
                        </div>
                    </div>
                </div>
                <div :class='{noDisplay: hideTime}' class='time-picker'>
                    <div class='hour-selector'>
                        <div id='j-hour' v-on:click='showHourSelector'>{{hour}}</div>
                        <div :class='{showSelector: hourSelectorVisible}' class='scroll-hider' ref='hourScrollerWrapper'>
                            <ul ref='hourScroller'>
                                <li :class='{active: index == hourIndex}' v-for="(h, index) in hours" v-on:click='setHour(index, true)'>{{h}}</li>
                            </ul>
                        </div>
                    </div>
                    ...
                    <div class='minute-selector'>
                        <div id='j-minute' v-on:click='showMinuteSelector'>{{minute}}</div>
                        <div :class='{showSelector: minuteSelectorVisible}' class='scroll-hider' ref='minuteScrollerWrapper'>
                            <ul ref='minuteScroller'>
                                <li :class='{active: index == minuteIndex}' v-for="(m, index) in minutes" v-on:click='setMinute(index, true)'>{{m}}</li>
                            </ul>
                        </div>
                    </div>
                ...
                <button class='okButton' type='button' v-on:click='setDate'>OK</button>
            </div>
        </div>
    </div>
